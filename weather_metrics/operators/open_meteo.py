...
class OpenMeteoMetricsOperator(BaseOperator):
    ...
    def execute(self, context: Any):
        ...
        ds = context["ds"]
        index = (
            response[self.frequency]["time"].index(f"{ds}T00:00")
            if self.frequency == "hourly"
            else response[self.frequency]["time"].index(ds)
        )

        result = {"time": ds, "id": self.city_data["id"]}
        for metric in self.metrics:
            result[metric] = (
                self._aggregate_values(response[self.frequency][metric], index)
                if self.frequency == "hourly"
